...
class BoardGame:
    def __init__(self, num_players):
        ...
        self.player_hotels = {player_name: None for player_name in self.players}  # Initialize hotel ownership to None
    ...
    def handle_hotel(self, player_name, hotel_type):
        hotel_info = self.hotels[hotel_type.capitalize()]
        ...
        if self.players[player_name] >= hotel_value:
            ...
            self.player_hotels[player_name] = hotel_type.capitalize()

    def get_asset_value(self, player_name):
        owned_hotel = self.player_hotels[player_name]
        return self.hotels[owned_hotel]['value'] if owned_hotel else 0
    ...
    def play(self, cells, dice_output):
        ...
        for i, player_name in enumerate(self.players):
            ...
            steps = dice_output[i]
            print(f"{player_name} rolled {steps}")
            if current_cell == "J":
                self.handle_jail(player_name)
            elif current_cell == "L":
                self.handle_lottery(player_name)
            elif current_cell == "H":
                hotel_type = input(f"{player_name}, choose hotel type (Sliver, Gold, or Platinum): ")
                self.handle_hotel(player_name, hotel_type)

            self.move_players(player_name, steps)
